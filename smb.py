...
def check_smb_service(host):
    try:
        ...
        if result.returncode == 0:
            print(f"SMB service on {host} is available. Shares:\n{result.stdout}")
            return True
        else:
            print(f"SMB service on {host} is not available. Error:\n{result.stderr}")
            return False
    except Exception as e:
        print(f"Error checking SMB service: {e}")
        return False
...
def get_smb_share(host):
    try:
        ...
        if result.returncode == 0:
            ...
            print(f"Error getting SMB shares: {result.stderr}")
            return False
    except Exception as e:
        print(f"Error getting SMB shares: {e}")
        return False
...
def check_cifs_mount(directory):
    ...
    if not is_mounted(directory):
        print(f"{directory} is not mounted.")
        return False
    ...
    try:
        ...
        if total is None:
            print(f"Failed to retrieve disk usage for {directory}.")
            return False
    except Exception as e:
    ...
    if used < 0:  # 假设如果已用空间超过 90%，认为挂载可能不正常
        print("Warning: High disk usage, check the mount.")
        return False
...
def restart_cifs_service():
    ...
    try:
        ...
        if stderr.channel.recv_exit_status() == 0:
            print("CIFS service restarted successfully.")
        else:
            print(f"Failed to restart CIFS service. Error: {error}")
...
def refresh_mount_cache(directory):
    print(f"Refreshing mount cache for {directory}...")
    # 执行 ls 命令来刷新挂载点状态
    try:
        subprocess.run(['ls', directory], check=True)
    except subprocess.CalledProcessError as e:
        print(f"Error while refreshing mount cache: {e}")